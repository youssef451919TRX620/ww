...
class emoji(commands.Cog):
	...
	async def on_ready(self):
		activity = discord.Game(name="All rights reserved ©2020", type=3)
		await self.bot.change_presence(status=discord.Status.online, activity=activity)
		tap = PrettyTable(
				['Name Bot', 'Id', 'guilds', 'commands'])
		tap.add_row([
	...
	async def on_message(self, message):
		...
		if ":" in message.content:
			...
			if em:
				...
				webhook = utils.get(webhooks, name = "© ")
				if webhook is None:
					webhook = await message.channel.create_webhook(name = "© ")
